...
int main(int argc, char *argv[])
{
  ...
  {
usage:
    fprintf(stdout, "HIP Bench - Async. Memory Transfers Throughput evaluation with NUMA consideration 1.0.0\n");
  ...
  else
  {
    ...
    else
    {
      if(hipAlloc)
      {
        snprintf(buff_memcpyasync_time, 100, "%lu-MB_numa_memcpyasync_time_copy_device_hip.csv", size_in_mbytes);
      }
      else
      {
  ...
  else
  {
    ...
    else
    {
      if(hipAlloc)
      {
        snprintf(buff_memcpyasync_gbs, 100, "%lu-MB_numa_memcpyasync_gbs_copy_device_hip.csv", size_in_mbytes);
      }
      else
      {
